...
main(){
  ...
  #convert $input -resize 1000x1000 $out_file #juse resize
    ...
    mkdir -p $out_dir
    convert $out_file -crop 2x2+$EXPAND_BORDER+$EXPAND_BORDER@ +repage +adjoin $out_dir/$clean_name"_%d.png"
    ...
    do
        tile_name="${tile%.*}"
        convert $tiles_dir/$tile -alpha set -virtual-pixel transparent -channel A -morphology Distance Euclidean:1,35\! +channel "$feathered_dir/$tile_name.png"
    done
    ...
    montage $feathered_dir/$clean_name'_0.png' $feathered_dir/$clean_name'_1.png' \
                    ...
                    -tile 2x2 -geometry -$border_w-$border_h $output/$clean_name.large_feathered.png
 # -$border_w-$border_h
    ...
    montage $tiles_dir/$clean_name'_0.png' $tiles_dir/$clean_name'_1.png' \
                    ...
                    -tile 2x2 -geometry -$border_w-$border_h $output/$clean_name.large.png
